...
<script setup lang="ts">
  ...
  import type {
    ...
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/RechargeTypes";
  import { getDataListApi, del, putMultiDel
  } from "/@/api/appinfo/RechargeApi";
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'userid', label: '用户编号',sortable:false},

                    {prop: 'createtime', label: '创建时间',sortable:true},

                    {prop: 'orderno', label: '订单编号',sortable:false},

                    {prop: 'amount', label: '充值金额',sortable:false},

                    {prop: 'status', label: '支付状态',sortable:true},

                    {prop: 'paytime', label: '支付时间',sortable:true},
  ...
  const getTableList = async () => {
    ...
    for (let i in queryForm) {
     ...
     // }
      //有时间段需自行获取值之后赋值相应属性
       if (arr.includes(i)) {
         if (i == "createtime" && queryForm[i]) {
           params.createtimestartTime = queryForm[i][0];
           params.createtimeendTime = queryForm[i][1];
         }
         else if (i == "paytime" && queryForm[i]) {
           params.paytimestartTime = queryForm[i][0];
           params.paytimeendTime = queryForm[i][1];
         } else {
  ...
  const getstatusSelectData =async () =>{
     let tempSelectData=[{label:"未支付",value:"未支付"},{label:"已支付",value:"已支付"},{label:"已取消",value:"已取消"}]
