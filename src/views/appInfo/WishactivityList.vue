...
<script setup lang="ts">
  ...
  import editDialog from "/@/views/appInfo/components/WishactivityDialog.vue";
  import lookDialog from "/@/views/appInfo/components/WishactivityDetail.vue";
  import lookWishDialog from "/@/views/appInfo/components/ActivityWishListDialog.vue";
  ...
  import type {
    ...
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/WishactivityTypes";
  import { getDataListApi, del, putMultiDel
  } from "/@/api/appinfo/WishactivityApi";
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'sname', label: '简称',sortable:false},

                    {prop: 'stitle', label: '标题',sortable:false},

                    {prop: 'snumber', label: '活动编号',sortable:false},

                    {prop: 'sharetype', label: '互助类型',sortable:false},

                    {prop: 'paywishpower', label: '消费心愿力',sortable:false},

                    {prop: 'joinnumber', label: '参加人数',sortable:false},

                    {prop: 'joincity', label: '参加城市',sortable:false},

                    {prop: 'addtime', label: '添加时间',sortable:false},

                    {prop: 'updatetime', label: '修改时间',sortable:false},

                    {prop: 'endtime', label: '截止时间',sortable:false},
  ...
  const getOperate = () => {
    operateBtn.value = getTopButton([
      ...
      { name: "查看愿望", funName: "lookWishClick" },
  ...
  const lookClick = (pid: number) => {
    ...
    //查看愿望事件
  const lookWishClick = (pid: number) => {
    debugger;
    ActivityWishListDialogilList.value.modeType = "look";
    ActivityWishListDialogilList.value.title = "查看参加愿望";
  ...
  const getTableList = async () => {
    ...
    for (let i in queryForm) {
      ...
      if (arr.includes(i)) {
         if (i == "addtime" && queryForm[i]) {
           ...
           params.endaddtime = queryForm[i][1];
         } else {
    ...
    if (res.code == 200) {
      ...
      const formattedData = data.map(item => ({
        ...
        sdetails: baseUtil.base64_decode(item.sdetails)
     }));
