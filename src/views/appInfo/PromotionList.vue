...
<script setup lang="ts">
  ...
  import type {
    ...
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/PromotionTypes";
  import { getDataListApi, del, putMultiDel
  } from "/@/api/appinfo/PromotionApi";
  ...
  // 创建下拉框对象数组
  const iscompleteSelectData = ref<any[]|[]>([])
    ...
    queryList.value = [
      {
        ...
        bindVal: 'promotername',
        curEleTag: "el-input",
        curOptions: {
             placeholder: translate('推广人'),
             clearable: true,
             autocomplete: "off",
             type: "text",
        },
      },
      {
        ...
        bindVal: 'promoteename',
        curEleTag: "el-input",
        curOptions: {
             placeholder: translate('被推广人'),
             clearable: true,
             autocomplete: "off",
             type: "text",
        },
      },
      {
        ...
        curOptions: {
              placeholder:translate('是否完成0-未完成 1-已完成'),
              clearable: true,
        },
        childOptions: iscompleteSelectData.value,
      },
  ]};
  //格式化表格某些字段的数据
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'promotername', label: '推广人',sortable:false},

                    {prop: 'promoteename', label: '被推广人',sortable:false},

                    {prop: 'createtime', label: '推广时间',sortable:false},

                    {prop: 'iscomplete', label: '是否完成',sortable:false,list:iscompleteSelectData.value },

                    {prop: 'remark', label: '说明',sortable:false},

                    {prop: 'updatetime', label: '完成时间',sortable:false},
  ...
  //下拉框数据赋值
  const getiscompleteSelectData =async () =>{
     let tempSelectData=[{label:"全部",value:-1},{label:"否",value:0},{label:"是",value:1}]
     ...
     iscompleteSelectData.value = arr
  }
  //生命周期hooks
  onMounted(async () => {
    await getiscompleteSelectData()
    await getQueryList();
    await getButton();
