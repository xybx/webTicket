...
<script setup lang="ts">
  ...
  import type {
    ...
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/ProrecordTypes";
  import { getDataListApi, updateStatus, batchUpdateStatus
  } from "/@/api/appinfo/ProrecordApi";
  ...
  const getButton = () => {
    ...
    { name: "礼品批量兑现", funName: "batchUpdateStatus" },
    ]) as ButtonProps[];
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'nickname', label: '兑换人昵称',sortable:false},

                    {prop: 'addtime', label: '兑换时间',sortable:false},

                    {prop: 'price', label: '兑换价格',sortable:false},

                    {prop: 'status', label: '处理状态',sortable:false },
                    {prop: 'operuser', label: '操作人',sortable:false},

                    {prop: 'opertime', label: '操作时间',sortable:false},
  ...
  const getOperate = () => {
    operateBtn.value = getTopButton([
      { name: "礼品兑现", funName: "updateStatus" }
    ]) as ButtonProps[];
  };
  ...
  //礼品批量兑现
  const batchUpdateStatusClick = async () => {
    
    let params = { proid: proid };
    let res: any = await batchUpdateStatus(params);
    if (res.code == 200) {
  ...
  //礼品兑现
  const updateStatusClick = async (pid: number) => {
    console.log(pid);
    ElMessageBox.confirm(`确定要执行吗?`).then(
        async () => {
          let res = (await updateStatus({ pid: pid })) as any;
          console.log(res);
          if (res.code == 200) {
            ElMessage.success("执行成功");
            getTableList();
  ...
  const map = new Map([
    ['addClick',addClick],
    ['batchUpdateStatus',batchUpdateStatusClick],
    ['searchClick',searchClick],
    ['resetClick',resetClick],
    ['editClick',editClick],
    ['lookClick',lookClick],
    ['updateStatus',updateStatusClick]
  ...
  const getTableList = async () => {
    let params: any = {proid:dialogList.value.pid, pageNum: pageNum.value, pageSize: pageSize.value,sortType:sortType.value,sortColumn:sortColumn.value };
  ...
  const getstatusSelectData =async () =>{
     let tempSelectData=[{label:"未兑现",value:"未兑现"},{label:"已兑现",value:"已兑现"}]
