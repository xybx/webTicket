...
import { autologin, getUserInfo, login, logout,Testlogin } from '/@/api/user'
...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    async autologin(userInfo: any) {
      const {data} = await autologin(userInfo)
      // 创建连接websocket
      // this.init(data.pid)
      this.afterLogin(data.token, tokenName)
    },
    async Testlogin(userInfo: any) {
      const {data} = await Testlogin(userInfo)
      // 创建连接websocket
      // this.init(data.pid)
      this.afterLogin(data.token, tokenName)
    },
